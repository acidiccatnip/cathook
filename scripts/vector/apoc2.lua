    ...
    [1] = {
        {
            ...
            mk_slider("fov radius",      "aim_fov",          100, 10, 500),
            mk_check("show fov circle",  "aim_drawfov",      true),
            mk_sec("sticky aim"),
            mk_check("enabled",          "aim_sticky",       false),
            mk_slider("sticky fov",      "aim_sticky_fov",   150, 10, 500),
            mk_color("locked color",     "aim_locked_color", {1, 0.3, 0.3, 1}),
            mk_sec("prediction"),
            mk_check("enabled",          "aim_predict",      false),
...
local function draw_zombie_skeleton(zombie, skel_color)
    local is_r15 = zombie:find_first_child("UpperTorso") ~= nil
...
local function run_aimbot(cam_pos)
    ...
    local aim_sticky = gui_get("aim_sticky")
    local sticky_fov = gui_get("aim_sticky_fov") or 150
    local aim_pred   = gui_get("aim_predict")
    local proj_speed = gui_get("aim_proj_speed") or 1000
    ...
    elseif key_down then
        ...
        for _, p in ipairs(entity.get_players()) do
            ...
            if dist < closest_dist then
                closest_dist=dist; best_x=sx; best_y=sy
            end
            ::ac::
        end
...
local function run_vehicle_esp(cam_pos)
    if not gui_get("vesp_on") then return end
...
local function run_zombie_esp(cam_pos)
    ...
    local ws = game.workspace
    if not ws or not utility.is_valid(ws) then return end
    local folder = ws:find_first_child("Zombies")
    if not folder or not utility.is_valid(folder) then return end
    ...
    for _, zombie in ipairs(folder:get_children()) do
        if not utility.is_valid(zombie) then goto zc end
...
local function run_corpse_esp(cam_pos)
    if not gui_get("cesp_on") then return end
    local ws = game.workspace
    if not ws or not utility.is_valid(ws) then return end
...
function on_frame()
    ...
    run_aimbot(cam_pos)
    run_vehicle_esp(cam_pos)
    run_zombie_esp(cam_pos)
    run_corpse_esp(cam_pos)
